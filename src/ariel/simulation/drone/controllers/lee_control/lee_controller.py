...
class LeeGeometricControl:
    ...
    def __init__(self, quad, yawType, orient="NED",
                 ...
                 matrix_gains=True,                         # Full inertia tensor (default since 2026-09-11)
        ...
        if auto_scale_gains and att_P_gain is None and rate_P_gain is None:
            ...
            if matrix_gains:
                ...
                # below, an asymmetric body gets a different closed-loop
                # natural frequency per mode: 22.66-25.60 rad/s against a 24
                # target for one arm swept 30 deg, 22.40-26.00 for one arm
                # lengthened to 0.26 m (docs/drone_morphology_gate_racing.md
                # section 7.2c, generated by docs/tools/principal_axes_table.py).
                # SUPERSEDED 2026-08-17: this comment used to quote
                # 23.37-24.69 and 22.11-26.03, from an unkept script.
                # Since 2026-09-10 the plant integrates the full inertia tensor,
                # so on an asymmetric body the diagonal form is a controller/
                # plant mismatch, not only a spread: the lengthened-arm body
                # flies 10.32 / 8.37 m/s at 60 / 90 deg with these gains
                # against 9.81 / 7.94 with the diagonal form (0.16 m/s
                # slower at 120 deg, unexplained; docs/plant_thrust_direction.md
                # section 9). matrix_gains defaults to True since 2026-09-11.
                # With zero products of inertia the two forms coincide exactly
                # (verified bit-identical in flight for the X quad).
