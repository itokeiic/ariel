...
% STALE 2026-09-16 (regenerated Table 1, gyroscopic plant) -- for the author.
% Contradicted: at 60 deg the narrowest frame is not last (the two widest are
% slower); at 120 deg it is LAST, and the widest is best; the optimum does not
% migrate toward the narrow end but from the middle (60) to the wide end (90,
% 120); the 120 deg ranking is not monotone (28.63 beats 36.81). See
% docs/drone_morphology_gate_racing.md section 2.
...
% STALE 2026-09-16 -- for the author. The spread no longer grows monotonically
% with sharpness: 4.1 / 32.7 / 25.8 % (largest at 90 deg).
...
% STALE 2026-09-16 -- for the author. The roll-authority mechanism predicts the
% narrow frames winning at sharp corners, which the regenerated data contradicts.
% The supported mechanism (pre-registered test, run on the pre-fix plant and not
% yet repeated) is the yaw component of Omega x I Omega opposing the demanded yaw
% on narrow frames beyond the rotors' yaw capacity; see the doc, section 8 item 4.
% CORRECTED 2026-09-17: repeated on the fixed plant and NOT confirmed (equal-push
% test refuted; docs/data/gyroscopic_mechanism_fixed_plant.md). The mechanism
% behind the regenerated ranking is currently unexplained -- do not cite one.
% "8.5 m/s on a 2.4 m leg" are literals from the old operating point.
...
reaches \SI{\PrevBestSixty}{\metre\per\second} at $60^\circ$ against
\SI{\PubBestSixty}{\metre\per\second} previously (\SI{-16.5}{\percent}),
\SI{\PrevBestNinety}{\metre\per\second} at $90^\circ$ (\SI{-2.5}{\percent}) and
\SI{\PrevBestOneTwenty}{\metre\per\second} at $120^\circ$ (\SI{-6.1}{\percent}).
% FIXED 2026-09-16: these three compared \Best* with \PubBest*. \Best* now holds
% the regenerated (gyroscopic-plant) bests, so the percentages would have been
% false; \PrevBest* holds the 2026-08-17 strict values this paragraph is about.
% STALE 2026-09-16: "unchanged in direction ... now sits at the narrow end with a
% monotone ranking" was true on the 2026-08-17 plant, not on the regenerated one.
...
\begin{figure}[htb]
  ...
  asked, by a mean of \SIrange{\XTrackMeanLo}{\XTrackMeanHi}{\metre} and a maximum of
...
% STALE 2026-09-16 -- for the author. (1) "34 deg of yaw amplitude against 60
% commanded, crabs 20 deg" and "+/-78 against +/-28" are literals from the old
% 90 deg flight (t=36.81 at its 2026-08-17 limit, \PrevBestNinety), which has
% been re-flown. (2) "the comparison
% is unaffected ... the lag is common-mode" no longer holds: with the gyroscopic
% term the yaw disturbance (Ixx-Iyy)pq differs across the family, and it is the
% mechanism behind the regenerated ranking. [CORRECTED 2026-09-17: that it drives
% the ranking was not confirmed on the fixed plant; only "not common-mode" holds.]
% Yaw authority per newton itself is
...
% STALE 2026-09-16: "the spread still grows with corner sharpness" is false on
% the regenerated data; the ranking-unchanged claim is still asserted by the build.
...
% STALE 2026-09-16 -- for the author. On the regenerated data the 60 deg column has
% a single best (not a plateau), and the 120 deg ranking is not monotone in t.
...
% OBSOLETE 2026-09-16 (later the same day): the plant now integrates the term and
% Table 1 is regenerated with it, so this caveat describes a limitation that no
% longer applies. Replace with a statement of what changed, or delete.
% 2026-09-17: its mechanism sentence ("opposes the yaw a slalom demands on narrow
% frames ...") was also not confirmed on the fixed plant.
...
the strict gate criterion; \texttt{speed\_semantics\_rankings.csv}, derived from
it by \texttt{docs/tools/speed\_semantics.py}; and the Figure~\ref{fig:flights}
flight logs in \texttt{docs/data/flight\_logs/}.
% Corrected 2026-09-16: this listed only tuned_strict_completion.csv, but
% make_figures.py also reads the rankings CSV and the three flight logs. The superseded sequential-criterion runs
