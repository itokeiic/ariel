...
\author{ARIEL / SPEAR}
\date{2026-09-17} % was 2026-08-17; corrected-model subsection added since
...
\begin{tabular}{@{}l l l@{}}
  ...
  $I_{xx}$ & moment of inertia about $x$ & \si{\kilo\gram\metre\squared} \\
  $I_{yy}$ & moment of inertia about $y$ & \si{\kilo\gram\metre\squared} \\
  $I_{zz}$ & moment of inertia about $z$ & \si{\kilo\gram\metre\squared} \\
  ...
  % DRAFT 2026-09-24: R ("corner radius", the three-gate circle) removed;
  % a_lat was "lateral acceleration demanded at speed $v$", i.e. v^2/R.
  $a_{\mathrm{lat}}$ & lateral acceleration of the reference, $|\dot p_{\rm ref}\times\ddot p_{\rm ref}|/|\dot p_{\rm ref}|$ & \si{\metre\per\second\squared} \\
  ...
  $\Omega_x$ & roll rate, the $x$ component of $\Omega$ & \si{\radian\per\second} \\
  $\Omega_y$ & pitch rate, the $y$ component of $\Omega$ & \si{\radian\per\second} \\
  $\Omega_z$ & yaw rate, the $z$ component of $\Omega$ & \si{\radian\per\second} \\
  ...
  $W$ & rotor angular speed & \si{\radian\per\second} \\
  $W_{\text{hover}}$ & rotor speed at hover & \si{\radian\per\second} \\
  $W_{\min}$ & idle speed of the motor model, the lowest it runs at & \si{\radian\per\second} \\
  $k_m$ & rotor drag-torque coefficient; a rotor's drag torque is $k_m W^2$ &
    \si{\newton\metre\second\squared} \\
...
%   \label{fig:courses}
% \end{figure}
% DRAFT (2026-09-24) -- for the author to reword. SUPERSEDED the same day, the
% author's text:
%   "... so
%   \begin{equation}
%     d = s\cos(\theta/2), \qquad A = \tfrac{s}{2}\sin(\theta/2), \qquad
%     R = \frac{s}{2\sin(\theta/2)}, \qquad a_{\mathrm{lat}} = \frac{v^2}{R},
%   \end{equation}
%   for gate spacing, lateral amplitude, corner radius, and the lateral
%   acceleration demanded at speed $v$. Holding $s$ fixed rather than $d$ is what
%   makes $R$ fall monotonically with $\theta$: at fixed spacing, a sharper turn
%   also lengthens the legs, and the corner ends up \emph{wider}."
% R is the circle through three consecutive gates, which the drone does not fly,
% and v^2/R is about 1.5x below the reference's demand at a typical gate. At
% fixed spacing the reference's tightest radius does fall monotonically. The
% reason that survives is course length. Sources:
% docs/drone_morphology_gate_racing.md section 5 (2026-09-24 note);
% SlalomCourse.reference_demand; macros from course_demand_vals in
...
\begin{equation}
  d = s\cos(\theta/2), \qquad
  A = \tfrac{s}{2}\sin(\theta/2),
\end{equation}

for gate spacing and lateral amplitude. Holding $s$ fixed rather than $d$ keeps the course the same length at every turn angle: the polyline through the waypoints is \SI{\CoursePoly}{\metre} on every course, and the reference flown along it \SIrange{\CourseArcLo}{\CourseArcHi}{\metre}. At a fixed spacing of \SI{2.0}{\metre}, the course would grow from \SI{\SpacingPolyLo}{\metre} at $\theta=60^\circ$ to \SI{\SpacingPolyHi}{\metre} at $120^\circ$, and difficulty would be confounded with flight length.

The demand is measured on the reference trajectory, not inferred from the gate layout. The reference is a cubic B-spline through the gates, timed uniformly in its parameter (Figure~\ref{fig:splineparam}), so it slows where it bends hardest. Its lateral acceleration $a_{\mathrm{lat}} = |\dot p_{\rm ref}\times\ddot p_{\rm ref}|/|\dot p_{\rm ref}|$ peaks at each gate. At a nominal speed of \SI{\DemandSpeed}{\metre\per\second}, the peak at a typical gate (the median over gates) is \SI{\DemSixty}{}, \SI{\DemNinety}{} and \SI{\DemOneTwenty}{\metre\per\second\squared} for $\theta=60^\circ$, $90^\circ$ and $120^\circ$. The last scored gate demands \SIrange{\DemLastLo}{\DemLastHi}{\percent} more than a typical gate, because as the reference nears the clamped end of the spline it both speeds up and bends more tightly.
...
% advantage.
Each airframe is scored by the maximum speed at which it still completes the course without missing a gate. The speeds are located by bisection. A gate counts as passed only if the drone's offset from the gate centre, measured \emph{in the gate plane at the crossing} and including its vertical component, is within the gate radius of \SI{0.5}{\metre}. That offset is a Euclidean norm, so the gate is a circular opening rather than the square frame the word ``gate'' usually denotes; a square of the same half-width encloses \SI{27}{\percent} more area, all of it near the diagonals, so the criterion used here is the stricter of the two.
% DRAFT (2026-09-24) -- for the author to reword. The plant has no gate or
% collision geometry (src/ariel/simulation/drone/plant.py, drone_simulator.py);
% the gates enter only the scoring in rollout().
Gates have no physical presence in the simulation: the test is applied to the drone's centre of mass, so the airframe's extent (rotor tips up to \SI{\RotorTipReach}{\metre} from the centre) is not checked against the opening.
Bisection is valid because completion is monotone in speed; the assumption is checked on every run and held in all cases reported here.

Two conditions matter for the numbers below. Feedforward of the reference velocity and acceleration is enabled (c.f. section \ref{sec:feedforward}), and the controller cascade (outer position loop and inner attitude loop) is tuned so the position loop sits well below the attitude loop ($\omega_n^{\mathrm{att}}=\SI{24}{\radian\per\second}$, $\omega_n^{\mathrm{pos}}=\SI{2.0}{\radian\per\second}$, $\zeta=1$). Under the inherited tuning, the loops were separated by only a factor of \num{3.2}, motor saturation sat at \SIrange{0.1}{2}{\percent}, and the \emph{controller} was the active constraint. In that regime, the same airframe wins on every course, and the spread across the family is \SI{3}{\percent}. % DRAFT 2026-09-17: model named in the next sentence; not re-measured on the corrected model.
At the tuned operating point in the reduced model of Section~\ref{sec:results}, every airframe saturates its motors \SIrange{15}{21}{\percent} of the time, so the airframe binds the performance instead.
...
% Restored 2026-09-17 from commit 8d3174b (the Results section as it stood before
% the corrected-model regeneration of 990594c). The model statement below, the
% "reduced model" in two captions, the cross-track macros in the Figure 3 caption
% and Section~\ref{sec:corrected} are new; the rest is the text of 8d3174b.
% DRAFT (2026-09-17) -- for the author to reword.
The results in this section were obtained with a \emph{reduced} simulation
model: its rigid-body dynamics omit the gyroscopic term of Euler's equations, its
rotor drag torque is linearised about hover, and the controller's lowest motor
speed is set below the motor model's idle speed.
Section~\ref{sec:corrected} repeats the measurements with those three items
corrected and reports what changes.

\begin{table}[htb]
  \centering
  \caption{Maximum completing speed (\si{\metre\per\second}) by arm half-angle
  and corner sharpness, reduced model. Bold marks the best in each column. Roll
  authority $\alpha_\phi$ is in \si{\radian\per\second\squared} per newton.}
  \label{tab:results}
...
\begin{figure}[htb]
  ...
  \caption{Reference against reality on the reduced model, each course flown by
  its own best airframe at its own limiting speed. Grey is the B-spline the
  controller is given; blue is the path actually flown; green bars are the
  fifteen scored gates, all passed at these operating points. The drone does not
  track the reference---it lags in phase and \emph{overshoots} the weave, flying
  a wider amplitude than asked, by a mean of
  \SIrange{\XTrackMeanLo}{\XTrackMeanHi}{\metre} and a maximum of
...
\subsection{Corrected rigid-body model}
\label{sec:corrected}

% DRAFT (2026-09-17) -- for the author to reword. Sources:
% docs/data/tuned_strict_completion.csv (Table 3), docs/data/flight_logs/
% (Figure 5), docs/data/gyroscopic_counterfactual.md (attribution),
% docs/data/gyroscopic_mechanism.md and gyroscopic_mechanism_fixed_plant.md
% (mechanism); account in docs/drone_morphology_gate_racing.md, section 8
% items 4 and 17.
% DRAFT 2026-09-17: was "Three simplifications of the reduced model"; the motor
% floor is a controller setting, not a simplification in the plant.
Three differences from a faithful model were found after the results above
were obtained, and the measurements were repeated without them.
Table~\ref{tab:models} lists what differs. Everything else is identical: the
controller and its gains, the motor mixer, the courses, the scoring and the
bisection. The two earlier model corrections, thrust along each rotor's own axis
and the full inertia tensor, change nothing for this family, whose rotors are
coplanar and axial and whose body axes are principal: the code at commit
\texttt{8d3174b}, which already contains both, reproduces every cell of
Table~\ref{tab:results}.

\begin{table}[htb]
  \centering
  \caption{What differs between the reduced and corrected models.}
  \label{tab:models}
  \small
  \begin{tabular}{@{}l l l@{}}
    \toprule
    & reduced (Table~\ref{tab:results}) & corrected (Table~\ref{tab:corrected}) \\
    \midrule
    gyroscopic term $\Omega\times I\Omega$ in the plant & omitted & integrated \\
    rotor drag torque, about each rotor axis & $2k_m W_{\text{hover}} W$ & $k_m W^2$ \\
    lowest motor speed the controller assumes & \SI{75}{\radian\per\second} &
      $W_{\min}$, \SI{238.49}{\radian\per\second} \\
    \bottomrule
  \end{tabular}
\end{table}

The three items, in order. The attitude controller adds $\Omega\times I\Omega$
to cancel the gyroscopic term; the reduced plant never integrated that term, so
the cancellation acted as a torque injected into the flight, one that differed
across the family. The controller allocates motor speeds assuming a drag torque of
$k_m W^2$, with the same $k_m$ the plant uses, but the reduced plant used the
tangent to that curve at hover, which delivers less yaw torque whenever the
motors run faster than hover, as they do in these corners. And the controller's
lowest motor speed, \SI{75}{\radian\per\second}, was a default inherited from
another simulator, below the idle speed $W_{\min}$ of the identified motor model;
commands below $W_{\min}$ were raised to it by the plant, so correcting it moves
no measured speed.

\begin{table}[htb]
  \centering
  \caption{Maximum completing speed (\si{\metre\per\second}) by arm half-angle
  and corner sharpness, corrected model. Bold marks the best in each column, as
  in Table~\ref{tab:results}.}
  \label{tab:corrected}
  \input{results_table_corrected.tex}
\end{table}

The corrected model reverses the trend at the sharper corners
(Table~\ref{tab:corrected}). At $60^\circ$ the middle of the family is still best,
at $\CorrArgmaxSixty^\circ$. At $90^\circ$ the best bodies are
$\CorrArgmaxNinety^\circ$, and at $120^\circ$ they are
$\CorrArgmaxOneTwenty^\circ$; the narrowest frame, tied first at $120^\circ$ in
Table~\ref{tab:results}, is now last at both, by
\SI{\CorrNarrowNinety}{\percent} and \SI{\CorrNarrowOneTwenty}{\percent}. The
spread across the family is \SI{\CorrSpreadSixty}{\percent},
\SI{\CorrSpreadNinety}{\percent} and \SI{\CorrSpreadOneTwenty}{\percent}: larger
than in the reduced model at the two sharper corners, and no longer growing
monotonically with sharpness. No single static geometry is best across the
difficulty range in either model; which end of the family wins at sharp corners
depends on the model.

The reversal comes from the gyroscopic term, not from the drag correction or the
mixer. With the drag correction alone, narrow-to-middle frames stay best:
$\CfQuadYawArgmaxNinety^\circ$ at $90^\circ$ and
$\CfQuadYawArgmaxOneTwenty^\circ$ at $120^\circ$. With the gyroscopic term and
the drag correction, and a mixer that gives up yaw first when the motors
saturate, the wide end still wins: $\CfYawLastArgmaxNinety^\circ$ and $\CfYawLastArgmaxOneTwenty^\circ$. This
also qualifies the reduced model's heading argument above: there, the
controller's cancellation was itself a yaw-coupled torque that varied across the
family, so the yaw channel was not strictly common-mode even then.

% DRAFT (2026-09-17, hypothesis-divergence session) -- for the author to reword.
% Sources: docs/data/gyroscopic_axis_attribution.md and gyroscopic_yaw_path.md;
% criteria pre-registered in the docstrings of docs/tools/gyroscopic_axis_attribution.py
% and gyroscopic_yaw_path.py; account in the doc, section 8 item 4. Every verdict
% below is asserted by Reports/make_figures.py.
\emph{Why} the wide frames win. Every frame in this family has the same
$I_{zz}$, and because rotors and arms lie in one plane, $I_{zz}$ is almost exactly
$I_{xx}+I_{yy}$. Euler's equations (Appendix~\ref{sec:euler}) then make the
gyroscopic accelerations about
roll and pitch nearly the same for every frame,
$\dot\Omega_x \approx -\Omega_y\Omega_z$ and $\dot\Omega_y \approx \Omega_x\Omega_z$,
with coefficients of magnitude \EulerRPMin{} to \EulerRPMax{}. Only the yaw
coupling depends on geometry,
\[
  \dot\Omega_z = \frac{I_{xx}-I_{yy}}{I_{zz}}\,\Omega_x\Omega_y ,
\]
with a coefficient that runs from $\EulerYawNarrow$ on the narrowest frame, through
zero on the X quad, to $+\EulerYawWide$ on the widest. It changes sign across the
family, and it acts on the axis the rotors control most weakly.
% DRAFT 2026-09-17. SUPERSEDED the same day: "Roll authority on its own does favour
% the narrow frames: without any gyroscopic coupling, the narrowest frame is the
% fastest of the three tested at 90 deg." The gap to the X quad is inside the
% bisection tolerance, a tie, and at 120 deg the X quad is ahead.
Two pre-registered tests on the corrected model trace the reversal to this yaw
coupling. The first kept the gyroscopic term, in both plant and controller, on
one body axis at a time, on the narrowest frame, the X quad and the widest
(Table~\ref{tab:axes}). Keeping it on every axis is the corrected model, and
reproduces Table~\ref{tab:corrected} cell for cell. The yaw axis alone reproduces
\SI{\GyAxisZLossNinety}{\percent} and \SI{\GyAxisZLossOneTwenty}{\percent} of the
narrow frame's loss at $90^\circ$ and $120^\circ$, and
\SI{\GyAxisZGainOneTwenty}{\percent} of the wide frame's gain at $120^\circ$.
Pitch reproduces at most \SI{\GyAxisYLossMax}{\percent} of the loss, and roll none
of it, % DRAFT 2026-09-17: the roll share of the gain was omitted
though roll alone reproduces \SI{\GyAxisXGainOneTwenty}{\percent} of the wide
frame's gain at $120^\circ$.
% DRAFT 2026-09-17, moved here 2026-09-22 from the paragraph above, where the
% numbers preceded the test that produced them (Table 4's "no axis" column).
Removing the coupling from every axis also answers the intuition that the narrow
frames, with their smaller $I_{xx}$, should suit a slalom: roll authority alone
does not make them faster on these courses. Without any gyroscopic coupling the
narrowest frame ties the X quad at $90^\circ$
(\SI{\GyNoneNarrowNinety}{\metre\per\second} against \SI{\GyNoneXNinety}{\metre\per\second},
inside the bisection tolerance) and trails it at $120^\circ$
(\SI{\GyNoneNarrowOneTwenty}{\metre\per\second} against
\SI{\GyNoneXOneTwenty}{\metre\per\second}).
The second test separated plant from controller on the yaw axis. The yaw torque
acting in the plant reproduces all of the narrow frame's loss
(\SI{\GyPlantLossNinety}{\percent} and \SI{\GyPlantLossOneTwenty}{\percent}), even
when the controller does not cancel it. The controller's cancellation acting
alone has the mirror effect: it raises the narrow frame's limit and lowers the
wide frame's. That mirror effect is a plausible but untested account of the
reduced model's opposite ranking, since its controller injected the term its
plant lacked.

\begin{table}[htb]
  \centering
  \caption{Maximum completing speed (\si{\metre\per\second}) with the gyroscopic
  term kept only on the listed body axes, in the plant and in the controller's
  cancellation alike. ``All axes'' is the corrected model of
  Table~\ref{tab:corrected}; ``no axis'' omits the term consistently, as the
  reduced model's plant did but its controller did not. Three of the seven
  airframes---the narrowest, the X quad and the widest---on the two sharper
  courses; the $60^\circ$ course, whose spread across the family is
  \SI{\CorrSpreadSixty}{\percent}, was not flown.}
  \label{tab:axes}
  \input{axis_attribution_table.tex}
\end{table}

How the yaw torque turns into missed gates is not established. Yaw saturation,
supported on the reduced model, was not confirmed on the corrected one: flown at
the same fraction of its own limit, the narrowest frame does not saturate yaw
most.
% DRAFT 2026-09-17. SUPERSEDED the same day: "Nor does the coupling act through
% heading lag or crab at the gates." The flights pass every gate, so they rule out
% growth below the limit, not a path above it; and the 3-D sideslip logged with
% them, unreported, is the one quantity that does grow.
Nor did heading lag or crab grow at the gates. Flown at the same speed with and
without the coupling, just below the narrow frame's limit, its heading error at
the gates changes by a factor of \GyHeadingNarrowNinety{} at $90^\circ$ and
\GyHeadingNarrowOneTwenty{} at $120^\circ$, and its crab angle by
\GyCrabNarrowNinety{} and \GyCrabNarrowOneTwenty{}, where growth was predicted.
These flights pass every gate, so they show the flight before it fails, not the
failure. One quantity logged in them without a prediction does grow: the narrow
frame's three-dimensional sideslip at the gates, from
\SIrange{\GyBetaNarrowNinetyN}{\GyBetaNarrowNinetyZ}{\degree} at $90^\circ$ and
from \SIrange{\GyBetaNarrowOneTwentyN}{\GyBetaNarrowOneTwentyZ}{\degree} at
$120^\circ$, against
\SIrange{\GyBetaWideNinetyN}{\GyBetaWideNinetyZ}{\degree} and
\SIrange{\GyBetaWideOneTwentyN}{\GyBetaWideOneTwentyZ}{\degree} on the widest
frame. That is a lead for the next test, not a result.

\begin{figure}[htb]
  \centering
  \includegraphics[width=\textwidth]{figures/flights_corrected.pdf}
  \caption{Reference against reality on the corrected model, each course flown by
  the first of its best airframes in Table~\ref{tab:corrected} at its own limiting
  speed; colours as in Figure~\ref{fig:flights}. The mean cross-track overshoot is
  \SIrange{\CorrXTrackMeanLo}{\CorrXTrackMeanHi}{\metre} and the maximum
  \SI{\CorrXTrackMax}{\metre}.}
  \label{fig:flights_corrected}
\end{figure}

\paragraph{Limits of the corrected results.} The motor mixer is unchanged: it
clips each motor independently, so a yaw demand beyond what the rotors can supply
also distorts thrust, roll and pitch; a mixer that gives up yaw first was tried
only as the check above. The quadratic drag form was chosen so the plant matches
the controller's allocation, whereas the identified motor model this plant was
ported from is linear in motor speed. The controller tuning was not revisited
for the corrected model.
% DRAFT 2026-09-17. SUPERSEDED the same day: "and the heading figures quoted above
% (yaw amplitude, sideslip, roll and pitch excursions) were measured on the reduced
% model only" -- contradicted by the corrected-model heading, crab and sideslip
% figures quoted just above.
The heading figures of Section~\ref{sec:results} (yaw amplitude, sideslip, roll
and pitch excursions) were not re-measured on it; only the comparison with and
without the yaw coupling above was flown on the corrected model.
...
Figures, the generated tables and every number quoted inline
...
latexmk -pdf -cd Reports/morphology\_gate\_racing.tex
\end{quote}
% Corrected 2026-09-16: this read `latexmk -pdf Reports/morphology_gate_racing.tex`,
% which fails from the repository root (numbers.tex not found) because the
% inputs are relative to Reports/. -cd builds in the file's own directory.

\begin{sloppypar}
Source data, all under the strict gate criterion. Reduced model
(Section~\ref{sec:results}): \texttt{tuned\_strict\_completion\_reduced\_model.csv},
\texttt{speed\_semantics\_rankings\_reduced\_model.csv} and the
Figure~\ref{fig:flights} logs in \texttt{docs/data/flight\_logs/reduced\_model/},
restored from commit \texttt{8d3174b}. Corrected model
(Section~\ref{sec:corrected}): \texttt{tuned\_strict\_completion.csv},
\texttt{speed\_semantics\_rankings.csv} and the Figure~\ref{fig:flights_corrected}
logs in \texttt{docs/data/flight\_logs/}; its attribution figures come from
\texttt{docs/data/gyroscopic\_counterfactual/}, and the cause of its reversal from
\texttt{docs/data/gyroscopic\_axis\_attribution/} and
\texttt{docs/data/gyroscopic\_yaw\_path/}. The rankings files are derived
from the tables by \texttt{docs/tools/speed\_semantics.py}.
% Corrected 2026-09-16: this listed only tuned_strict_completion.csv, but
% make_figures.py also reads the rankings CSV and the three flight logs.
% Corrected 2026-09-17: the 2026-09-16 comment above ended on the same line as
% "The superseded sequential-criterion runs", which it commented out of the PDF;
% and the report now draws on two model versions, listed above.
The superseded sequential-criterion runs
...
\section{Euler's equation of motion}\label{sec:euler}

% DRAFT 2026-09-25: notation preamble, so this appendix is self-contained and its
% symbols are not read against the Notation section of the body.
This appendix keeps the standard flight-dynamics notation, which is not the
notation of the body. Here $\boldsymbol{F}$ and $\boldsymbol{G}$ are the body
force and moment, $(U,V,W)$ the body-axis velocity components and $(P,Q,R)$ the
body-axis angular rates---the rates the body writes $\Omega_x$, $\Omega_y$,
$\Omega_z$. \emph{In this appendix only, $W$ is a velocity}; everywhere else in
the report $W$ is a motor speed in \si{\radian\per\second}. The moments of
inertia $\boldsymbol{I}_{xx}$, $\boldsymbol{I}_{yy}$, $\boldsymbol{I}_{zz}$ are
those of the body's Notation section, and $\boldsymbol{I}_{xy}$,
$\boldsymbol{I}_{xz}$, $\boldsymbol{I}_{yz}$ are the products of inertia, which
are identically zero for the airframes flown here.

The terms quadratic in the rates are the gyroscopic coupling. Dropping them
leaves $\boldsymbol{I}\dot{\boldsymbol{\Omega}}=\boldsymbol{G}$, which is the
form the reduced model integrated (Table~\ref{tab:models}).

\begin{equation}
\begin{aligned}
& \boldsymbol{F}=\boldsymbol{F}_x \boldsymbol{i}+\boldsymbol{F}_y \boldsymbol{j}+\boldsymbol{F}_z \boldsymbol{k} \\
& \boldsymbol{G}=\boldsymbol{G}_x \boldsymbol{i}+\boldsymbol{G}_y \boldsymbol{j}+\boldsymbol{G}_z \boldsymbol{k} \\
& \left\{\begin{array}{l}
m\left(\frac{d U}{d t}+Q W-R V\right)=\boldsymbol{F}_x \\
m\left(\frac{d V}{d t}+R U-P W\right)=\boldsymbol{F}_y \\
m\left(\frac{d W}{d t}+P V-Q U\right)=\boldsymbol{F}_z
\end{array}\right. \\
& \left\{\begin{array}{l}
\boldsymbol{I}_{x x} \frac{d P}{d t}-\boldsymbol{I}_{x y} \frac{d Q}{d t}-\boldsymbol{I}_{x z} \frac{d R}{d t}-\boldsymbol{I}_{x z} P Q+\left(\boldsymbol{I}_{z z}-\boldsymbol{I}_{y y}\right) Q R+\boldsymbol{I}_{y x} R P-\boldsymbol{I}_{z y} Q^2+\boldsymbol{I}_{y z} R^2 \\
\quad=\boldsymbol{G}_x \\
-\boldsymbol{I}_{y x} \frac{d P}{d t}+\boldsymbol{I}_{y y} \frac{d Q}{d t}-\boldsymbol{I}_{y z} \frac{d R}{d t}+\boldsymbol{I}_{z y} P Q-\boldsymbol{I}_{x y} Q R+\left(\boldsymbol{I}_{x x}-\boldsymbol{I}_{z z}\right) R P+\boldsymbol{I}_{z x} P^2-\boldsymbol{I}_{x z} R^2 \\
% 2026-09-25: the coefficient of RP above was pasted as (I_xx - I_zx); derived
% from G = dH/dt + Omega x H it is (I_xx - I_zz). F_y below the force rows was
% pasted as F_{yy}.
=\boldsymbol{G}_y \\
-\boldsymbol{I}_{z x} \frac{d P}{d t}-\boldsymbol{I}_{z y} \frac{d Q}{d t}+\boldsymbol{I}_{z z} \frac{d R}{d t}+\left(\boldsymbol{I}_{y y}-\boldsymbol{I}_{x x}\right) P Q+\boldsymbol{I}_{x z} Q R-\boldsymbol{I}_{y z} R P-\boldsymbol{I}_{y x} P^2+\boldsymbol{I}_{x y} Q^2 \\
=\boldsymbol{G}_z
\end{array}\right.
\end{aligned}
\end{equation}
