...
\emph{Why} the wide frames win is not established. On the reduced model, a
pre-registered test supported a yaw mechanism: the yaw component of the
gyroscopic term opposes the yaw a slalom demands on narrow frames, beyond what
the rotors can supply. On the corrected model the same test was inconclusive on
its correlation criterion and refuted on its saturation criterion. A confirmatory
test that flew every airframe at the same fraction of its own limit refuted its
central prediction, that the narrowest frame saturates yaw most; its causal part,
removing the controller's cancellation, was inconclusive.
...
\texttt{docs/data/gyroscopic\_counterfactual/}. The rankings files are derived
