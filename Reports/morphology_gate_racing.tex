...
numbers, \texttt{GateChecker}, measures lateral error in the horizontal plane
only, so altitude is never checked and a drone that sinks below the gates still
records passes. Re-running with completion requiring three-dimensional proximity
to every gate lowers the $60^\circ$ column by \SIrange{8.3}{17.8}{\percent}, and
that column then spans only \SIrange{9.926}{10.137}{\metre\per\second}---a
\SI{2.1}{\percent} spread instead of \SI{12}{\percent}. The claims below about
the $60^\circ$ column, including the widest frame being competitive there, do
\emph{not} survive that correction. The $90^\circ$ and $120^\circ$ columns move
by less than the bisection tolerance and stand, so the corner-sharpness trend
between them is unaffected. Details in
