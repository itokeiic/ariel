...
#### The current headline result (full rigid-body plant, 2026-09-16)
...
`docs/tools/headline_table.py` from `docs/data/tuned_strict_completion.csv`, which
was regenerated at commit `4229360`. The plant now integrates the gyroscopic term
and quadratic rotor drag, and the controller's motor floor matches the plant's
(§8 items 4 and 17).
...
* **60°:** the middle wins (t=36.81°, 10.398 m/s) with a spread of 4.1%. The
  ends are not ordered: the narrowest frame (10.086) beats the two widest (9.969,
  10.008).
* **90°:** the X quad and the widest frame tie (45.00° at 8.250, 69.56° at
  8.211). Every frame narrower than 45° falls away, down to 5.555 at 20.44°
  (-32.7%).
* **120°:** the three widest frames tie (53.19-69.56°, best 7.117 at 69.56°),
  and the narrowest is last at 5.281 (-25.8%). The ranking is not monotone:
  28.63° (6.375) beats 36.81° (6.219).

So the optimum moves from the middle at 60° to the wide end at 90° and 120°. That
is the opposite end from the 2026-08-17 result below, measured on a plant without
the gyroscopic term.

"No single static geometry is best across the difficulty range" still holds. The
reason is now the wide end at sharp corners, not the narrow end.

The mechanism test (§8 item 4) attributes the narrow frames' loss to the yaw
component of $\Omega\times I\Omega$: it opposes the demanded yaw and exceeds the
...
subsection presents the corrected-model result above, with a table of what
...
> limits on this", were measured on a plant that omitted $\Omega\times I\Omega$
> and used hover-tangent yaw drag. They are kept as measured. The `current`
> variant of `docs/tools/gyroscopic_counterfactual.py` reproduces them at commit
> `8d3174b`.
...
### 4.1 What actually simulated these results, and porting to PhysX

Worth stating plainly, because the consortium's tooling runs on Isaac Lab and
the two are easy to conflate: **none of the results in this document were
produced by a general physics engine.** `examples/spear/19_morphology_design_sweep.py`
imports no `isaacsim`, no `pxr`, no MuJoCo. The plant is ARIEL's own reduced
model -- `DroneInterface` wrapping `DroneSimulator`, whose equations of motion
...
plane, and the family has no motor cant. That is why the sweep was confined to
azimuth and arm length in the first place.

The evolutionary search space is a different matter. Under example 17's own
...
**This is the difference between "a limitation to fix eventually" and "a bound on
...
`DronePlant` (§4) is the seam, deliberately small: `reset`, `state`,
`rotor_frames`, `mass_inertia`, `step`, plus rotor and joint counts.
`DroneCommand` carries **per-rotor thrust in newtons** rather than motor speeds,
so the mapping to whatever an engine wants belongs to the adapter. The
controller, trajectory generator, gate scoring and max-speed objective are all
backend-agnostic and transfer unchanged. Five things an `IsaacDronePlant` owns:

1. **Asset.** `blueprint_to_usd` already emits articulated USD -- rigid links
   under a `PhysicsArticulationRootAPI`, wired by `PhysicsRevoluteJoint` with
   drives -- using link and joint names that match the consortium's Isaac
   scripts, so they drive blueprint output unmodified.
2. **Frames.** These experiments run NED; `DroneState` and Isaac are Z-up. The
   conversion belongs at the boundary, once, not scattered through the loop.
3. **Where force is applied.** Applying one lumped wrench at the base body is
   exact for a *rigid* airframe and wrong for an articulated one: rotor forces
   physically act on the arm links, so lumping them at the base discards the
   joint reaction torques -- which is the part morphing is about. Per-rotor
   forces at each rotor body let the engine compose them correctly.
4. **Timing.** Control runs at 200 Hz here; a 10 ms physics step needs an
   explicit decimation policy rather than an implicit one.
5. **What the engine will not model.** Motor lag and aerodynamic drag are in the
   reduced plant and are not in a rigid-body solver. Dropping them silently is
   the easiest way to make two backends disagree on an airframe both should
   handle identically.

One piece of glue the protocol does not yet cover: the Lee controller reads a
legacy attribute surface (`quad.pos`, `quad.vel`, `quad.quat`, `quad.omega`,
`quad.euler`, `quad.params`), not `DronePlant`. An adapter needs a thin shim
exposing those, or `DroneInterface` gets re-based onto `DronePlant`.

#### The rule that keeps both stacks correct

For a morphing airframe, rotor position, thrust axis, mass and inertia are all
functions of the joint configuration `q`. Caching them at construction
reproduces §3.2's defect -- an allocation `B(q)` that no longer matches the
vehicle being simulated -- in a new place, and it surfaces exactly when the arms
start moving, which is when it is hardest to notice.

This is not hypothetical. The consortium's own wrench model computes the wrench
correctly, honouring each rotor's direction
(`f_i_b = thrust * rotor_dir_b`, moments by `cross(rotor_pos_b, f_i_b)`) -- but
it loads `rotor_positions` and `rotor_directions` once from saved metadata and
repeats them across environments. Static geometry is right for a fixed airframe
and goes stale the moment a joint moves. `DronePlant` therefore specifies that
`rotor_frames()` and `mass_inertia()` report the **current** configuration, so
`B(q)` is rebuilt each step rather than cached. Agreeing that rule on both sides
is what keeps the two stacks from diverging.
...
|---|---|---|---|---|
| symmetric X quad (swept family) | 10.203 m/s | 8.250 m/s | 6.688 m/s | axial |
| arm 0 azimuth +30 deg | 9.148 m/s | 7.273 m/s | 5.984 m/s | axial |
| arm 0 lengthened to 0.26 m | 9.852 m/s | 7.977 m/s | 6.609 m/s | axial |
| arm 0 elevated 15 deg (out of plane) | 9.969 m/s | 7.820 m/s | 6.570 m/s | axial |

*(Table regenerated 2026-09-16 on the full rigid-body plant; see the note at the
end of this section. The 2026-09-11 values it replaced are in the "Regenerated
2026-09-11" note's own table below.)*
...
>
> **Measured, regenerated 2026-09-16** (commit `4229360`, full rigid-body plant;
> `docs/data/perturbation.csv`, `docs/data/perturbation_table.md`):
>
> | perturbation | 60° | 90° | 120° | vs symmetric |
> |---|---|---|---|---|
> | symmetric X quad | 10.203 | 8.250 | 6.688 | -- |
> | arm 0 azimuth +30 deg | 9.148 | 7.273 | 5.984 | -10.3% / -11.8% / -10.5% |
> | arm 0 lengthened to 0.26 m | 9.852 | 7.977 | 6.609 | -3.4% / -3.3% / -1.2% |
> | arm 0 elevated 15 deg | 9.969 | 7.820 | 6.570 | -2.3% / -5.2% / -1.8% |
>
> - **Azimuth asymmetry** now costs about the same at every turn angle,
>   -10.3% / -11.8% / -10.5%, against -9.2% / -14.2% / -17.7% on the previous
>   plant.
> - **The lengthened arm** is now slower than the symmetric quad at every turn
>   angle. On the previous plant it was faster (+1.5% / +1.4% / +4.9%).
> - **The elevated arm** costs -2.3% / -5.2% / -1.8% (previously -2.3% / -6.6% /
>   -1.8%).
> - The symmetric quad equals Table 1's t=45° column.
...
> **SUPERSEDED IN PART 2026-09-16.** The text above is kept as written.
> * **"Does not confound" no longer holds.** Yaw authority per newton is still
>   identical across the family. But the plant now integrates the gyroscopic
>   term, whose yaw component $(I_{xx}-I_{yy})\,pq$ differs in sign and size
>   across the family.
>   * The pre-registered mechanism test (§8 item 4, run on the previous plant)
>     found that this component opposes the demanded yaw on narrow frames and
>     assists it on wide ones. On the outer frames it exceeds the 0.092 N·m yaw
>     capacity.
...
>   8.250 m/s. The table has not been re-measured.
> * **Commanded peak yaw rates** at the regenerated Table 1 bests are 658 / 937 /
>   1434 °/s (`docs/tools/yaw_authority.py`), not 647 / 947 / 1359.
...
best operating point. These are the regenerated Table 1 bests (§2); the tool
reads them from the CSV since 2026-09-16.
...
> **Regenerated 2026-09-16** at the new Table 1 bests. The previous tables, at
> 10.164 / 8.367 / 6.648 m/s, are kept here:
>
> | course | `--speed` | ref speed at gates (min/median/max) | peak ref speed | nominal / median |
> |---|---|---|---|---|
> | 60° | 10.164 m/s | 3.81 / 5.51 / 6.49 m/s | 28.9 m/s | 1.84x |
> | 90° | 8.367 m/s | 3.09 / 3.90 / 5.39 m/s | 28.7 m/s | 2.14x |
> | 120° | 6.648 m/s | 2.07 / 2.31 / 4.32 m/s | 26.7 m/s | 2.88x |
>
> In the decomposition they read startup 1.75x / 1.59x / 1.46x, parameterisation
> 1.05x / 1.35x / 1.97x, and total_time 6.778 / 7.589 / 8.776 s. Each factor
> moves by at most 0.03x and each product by at most 0.04x, so the reading
> of this section is unchanged. The next paragraph quoted 8.367 m/s, 1.84x at
> 60°, 2.88x at 120° and a peak of ~28 m/s.

So a body reported at **8.250 m/s** on the 90° course crosses its gates at
about **3.9 m/s**, and the discrepancy grows with corner sharpness: 1.86x at
60°, 2.92x at 120°. The peak reference speed of 28-29 m/s sits in the final span,
...
| Report Figure 3, reference vs flown | `docs/data/flight_logs/slalom_{60,90,120}deg.npz` | `--video --turn-deg {60,90,120} --video-half-angle {36.813816210732824,45.0,53.18618378926718} --video-speed {10.3984375,8.25,7.078125} --completion strict --att-omega-n 24 --pos-omega-n 2.0 --pos-zeta 1.0 --feedforward --max-accel 40 --log-npz <path>`, drawn by `Reports/make_figures.py`. **Re-flown 2026-09-16** on the full rigid-body plant (commit `4229360` plus the `--log-npz` fix below, committed with these logs). Each is the first body among its Table 1 ties at its own limit, and each passes 15/15 strict. Previously `--video-half-angle {36.81382,36.81382,20.44145} --video-speed {10.164,8.367,6.648}`. *(Corrected 2026-09-17: the command this row gave for those earlier logs omitted the controller flags. Flown literally as written, the 60° flight passes only 3/15 gates. With `--att-omega-n 24 --pos-omega-n 2.0 --pos-zeta 1.0 --feedforward --max-accel 40` it reproduces them; see the reduced-model row below.)* `--log-npz` had stopped saving the `half_angle_deg` and `speed` that `Reports/make_figures.py` reads, so the old command could not reproduce the committed logs; it saves them again as of 2026-09-16. |
| Report Table 1 and Figure 3, reduced model (restored 2026-09-17) | `docs/data/tuned_strict_completion_reduced_model.csv`, `docs/data/speed_semantics_rankings_reduced_model.csv`, `docs/data/flight_logs/reduced_model/slalom_60deg.npz` and its 90° and 120° siblings | Restored from commit `8d3174b`, where they were `tuned_strict_completion.csv`, `speed_semantics_rankings.csv` and the Figure 3 logs. **Table 1:** the counterfactual's `current` variant reproduces every cell at `8d3174b`. **Rankings:** `uv run --no-sync python docs/tools/speed_semantics.py --table docs/data/tuned_strict_completion_reduced_model.csv --rankings-out <path>` reproduces the file on current code, since the reference trajectory does not depend on the plant. **Flight logs:** at `8d3174b`, `--video --turn-deg {60,90,120} --video-half-angle {36.81382,36.81382,20.44145} --video-speed {10.164,8.367,6.648} --completion strict --att-omega-n 24 --pos-omega-n 2.0 --pos-zeta 1.0 --feedforward --max-accel 40` reproduces `t`, `pos`, `ref` and `euler` to the float32 precision they are stored at (max difference 1e-6). All three verified 2026-09-17. |
...
**Reproducibility check (2026-09-16).** Every result regenerated on 2026-09-16
was reproduced from a clean checkout of commit `4229360` plus the change set that
records it. Each was rebuilt with the commands in the table above and compared
with the committed files.

* **Sweep data:** `docs/data/tuned_strict_completion.csv` (21 cells) and
  `docs/data/perturbation.csv` (12 cells) are identical.
* **Figure 3:** the three flight logs in `docs/data/flight_logs/` are identical,
  array by array.
* **Generated from those, all identical** (the CSV up to line endings, which git
  normalises):
  * `docs/data/headline_strict_table.md`
  * `docs/data/perturbation_table.md`
  * `docs/data/perturbation_morphologies.png`
  * `docs/data/speed_semantics.md`
  * `docs/data/speed_semantics_rankings.md`
  * `docs/data/speed_semantics_rankings.csv`
  * `docs/data/yaw_authority.md`
  * `Reports/numbers.tex`
  * `Reports/results_table.tex`
* **Gyroscopic counterfactual**, spot-checked at commit `8d3174b`:
  * the `current` and `physical_quadyaw` variants are identical on all 21 cells;
  * `docs/data/gyroscopic_mechanism.md` is identical;
  * the other seven variants were not re-run.

The check also found three records wrong, corrected in place:
* the §3.8 row did not fix `--sweep-turns` or the speed bracket;
* the §7.2d row named the wrong default flags;
* the report's rebuild command failed from the repository root and now uses
  `latexmk -pdf -cd`.
...
1. **Normal-aware plant (§3.2) -- CLOSED 2026-09-11** (fixed 2026-09-10, commit
   ...
   *The priority, and more urgent than first judged.*
   ...
   under example 17's own genome limits **every** sampled body tilts its thrust
   axis, median 22.3°, with 38% of each rotor's thrust allocated and never
   applied. The azimuth sweep is exactly unaffected, so nothing published here
   changes; but this bounds every evolutionary result the pipeline produces.
   The fix is more contained than the analysis suggests: `DroneSimulator`
   already holds correct, normal-aware `Bf`/`Bm` from `DroneConfiguration` and
...
4. **The plant omits the gyroscopic term the controller cancels (§7.2b) -- FIXED
   2026-09-16 (commit `4229360`). Measured 2026-09-11; record corrected 2026-09-16.**

   *Fixed.* Both plants integrate $I^{-1}(M-\Omega\times I\Omega)$. Tests:
   `tests/unit/test_simulation/test_plant_thrust_direction.py` (Euler's equations on
   a torque-free rectangular quad). Table 1 and the perturbation table were
   regenerated on it (§2, §7.2d). The text below, written while the item was open,
   is kept.
...
9. **DECIDED (2026-09-11) -- speeds stay nominal (§7.6b).** `--speed` is not
   ...
   *Why not fixing it loses nothing within a column.* Table 1 was re-expressed
   ...
   > **Regenerated 2026-09-16** (Table 1 on the full rigid-body plant).
   > * Rankings by gate speed are still identical to those by nominal speed at
   >   every turn.
   > * The spreads are now 4.13 / 32.67 / 25.80% nominal and 2.97 / 26.83 /
   >   21.37% in gate speed. They no longer grow with sharpness, so the build no
   >   longer asserts that they do.
   > * The nominal-to-gate ratios are 1.86x / 2.14x / 2.92x.
   >
   > The paragraphs above are kept as measured on the previous plant.
...
17. **FIXED 2026-09-16 -- yaw drag torque was the hover linearisation** (added
   ...
   **FIXED 2026-09-16 (commit `4229360`).** Rotor drag torque is $k_m W^2$ about
   each rotor axis in both plants. That is the form the controller's mixer
   allocates with, using the same $k_m$.
   * *Why:* in Table 1 flights the hover tangent $2k_m W_{\text{hover}} W$ gave a
     median 0.68-0.78 of the quadratic yaw torque (5th percentile 0.50), because
     motors run 1.2-1.4x hover speed.
   * *The choice:* the identified 5-inch model the plant was ported from
     (`params_5inch` in randomization.py of tudelft/optimal_quad_control_RL, an
     external repository) is
     linear in $W$. Quadratic was chosen for controller/plant consistency.
   * *Evidence:* the counterfactual showed the Table 1 reversal survives either
     yaw model (`docs/data/gyroscopic_counterfactual.md`, `*_quadyaw` variants).
   * *Also fixed:* the controller's motor floor `minWmotor` was 75 rad/s, a
     Quadcopter_SimCon default for another drone. It is now the plant's
     identified idle speed `w_min` (238.49 rad/s). That changed no max speed:
     the plant already raised such commands to its floor. It does change the
     `saturation_lo` statistics.
   * *Tuning:* the controller tuning was not revisited.
